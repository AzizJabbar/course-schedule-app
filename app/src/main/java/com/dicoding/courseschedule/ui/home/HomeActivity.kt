...
// 15 : Write UI test to validate when user tap Add Course (+) Menu, the AddCourseActivity is displayed
class HomeActivity : AppCompatActivity() {

    private lateinit var viewModel: HomeViewModel
    private var queryType = QueryType.CURRENT_DAY

    // 5 : Show today schedule in CardHomeView and implement menu action
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.getQueryType().observe(this){ query ->
            val course = DataRepository.getInstance(this)?.getNearestSchedule(query)
            course?.observe(this){ it ->
